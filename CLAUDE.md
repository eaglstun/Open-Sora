...
- **Perf dead-ends (measured 2026-07-11, don't relitigate):** (1) **fp16 is out** — `--dtype fp16` produces **black frames** (the 11B MMDiT overflows fp16's ~65504 range → NaN; bf16 keeps fp32's exponent range, which is why it's the default), and it isn't even faster than bf16 on MPS. Keep `dtype = "bf16"`. (2) **Thermal throttling dominates timing** — two back-to-back 13f/20-step renders drifted 155s→205s (+32%) as the Mac heated and the GPU downclocked; this is larger than most micro-opts, so any A/B needs cooldowns between runs or the numbers lie. (3) **Torch upgrade is blocked by torchvision, not torch/MPS** — tested 2026-07-11 in an isolated venv: **torch 2.13 passes the MPS parity test** (SDPA + float32 rope numerically clean on new torch), but every torch ≥2.12 forces **torchvision ≥0.27, which deleted the `torchvision.io.video` API** (`read_video`/`write_video`/`_check_av_available`) that `opensora/datasets/read_video.py` + `datasets/utils.py` import at module load. tv 0.27 removed the module; tv 0.28 (torch 2.13's pair) re-added it as a **broken fbcode stub** (`from pytorch.vision.fb.io.video import …`), and 0.28.0 is the latest (no fix). **Prerequisite for any torch>2.10:** port opensora's video read/write off `torchvision.io.video` onto pyav (`av`, already installed) or torchcodec — a discrete ~2-file job, not a version bump. Only then can newer-torch MPS speed be measured. Remaining untested lever after that: attention-formulation tweaks.
